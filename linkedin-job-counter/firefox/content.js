/**
 * Content Script for LinkedIn Job Insights Extension (Firefox)
...
(function() {
  ...
  class LinkedInJobInsightsContent {
    ...
    async initializeExtension() {
      ...
      try {
        ...
        // Initialize UI injector using the global class
        this.uiInjector = new window.LinkedInUIInjector();
    ...
    async handleUrlChange() {
      ...
      if (this.isLinkedInJobPage()) {
        await this.sleep(2000); // Wait for LinkedIn to load new content
    ...
    async requestJobDataForCurrentPage() {
      try {
        ...
        const jobId = this.extractJobId(this.currentUrl);
        ...
        console.error('[LinkedIn Job Insights] Error requesting job data:', error);
      }
    }
    ...
    handleJobData(jobId, data) {
      ...
      console.log('[LinkedIn Job Insights] Received job data in content script:', jobId, data);
    ...
    handleMessage(message, sender, sendResponse) {
      try {
        ...
        console.error('[LinkedIn Job Insights] Error handling message:', error);
      }
    }
  ...
  browser.runtime.onMessage.addListener((message, sender) => {
    contentScript.handleMessage(message, sender);
  });
  ...
  console.log('[LinkedIn Job Insights] Firefox content script loaded');
